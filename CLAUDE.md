...
- Nothing currently stops someone from calling `emailjs.send` directly from devtools with the exposed IDs — EmailJS's **Allowed origins** domain allowlist (the real fix) is gated behind their Professional plan, confirmed 2026-07 on emailjs.com/pricing. Free-tier options if abuse ever shows up: EmailJS's template-level reCAPTCHA/hCaptcha setting (not plan-gated as far as we've found), watching the EmailJS dashboard and regenerating the Public Key if abused, or upgrading. Worst case on the current setup is quota exhaustion (200 free emails/month) and inbox spam, not a data breach — the OAuth link to the inbox never leaves EmailJS's servers.
...
- User input from Pager (message textarea, plus the visitor's own name/contact fields) is capped in length before it's stored or sent; all of it is passed to EmailJS as template variables (not rendered as HTML anywhere), so there's no injection surface beyond what the EmailJS template itself does with it. The name/contact fields exist so a reply is possible — they only reach the recipient (and only enable a real Reply-To) if the EmailJS template references `{{name}}`/`{{email}}` and has its "Reply To" setting pointed at `{{email}}`, which is a one-time dashboard change outside this repo.
